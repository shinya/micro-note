<template>
  <div
    class="custom-window-bar bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700"
  >
    <div class="flex items-center justify-between px-4">
      ...
      <div class="flex items-center space-x-2">
        ...
        >
          <svg
            class="w-2 h-2 text-red-500 group-hover:text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200"
            fill="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        >
          <svg
            class="w-2 h-2 text-yellow-500 group-hover:text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200"
            fill="currentColor"
            viewBox="0 0 24 24"
          >
            <path d="M6 12h12" />
        ...
        >
          <svg
            class="w-2 h-2 text-green-500 group-hover:text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200"
            fill="currentColor"
            viewBox="0 0 24 24"
          >
            <path
      ...
      <div class="flex-1 text-center drag-area" data-tauri-drag-region="true">
        <h1
          class="app-title text-sm font-semibold text-gray-800 dark:text-white py-1"
        >
          Micro Note
        </h1>
      </div>
...
const closeWindow = async () => {
  if (isTauriEnvironment.value && appWindow.value) {
    try {
...
const maximizeWindow = async () => {
  if (isTauriEnvironment.value && appWindow.value) {
    try {
      ...
      // ウィンドウの最大化/復元時にエラーが発生
    }
  } else {
    // ブラウザ環境ではウィンドウの最大化/復元ができません
  }
}
</script>
