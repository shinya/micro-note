...
const selectedLabelFilter = ref(undefined) // undefined = すべて表示
...
<template>
  <div
    class="app-container min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900"
  >
    <!-- カスタムウィンドウバー -->
    <WindowBar />
    ...
    <!-- ヘッダー -->
    <header
      class="fixed-header bg-white/80 backdrop-blur-md shadow-lg border-b border-white/20 z-40 dark:bg-gray-800/80 dark:border-gray-700"
    >
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
          <div class="flex items-center space-x-3">
            <div
              class="w-8 h-8 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl flex items-center justify-center"
            >
              <svg
          ...
          <div class="flex items-center space-x-2 flex-1 max-w-sm ml-6">
            <div class="relative flex-1">
              ...
              />
              <svg
                class="absolute left-2.5 top-1/2 transform -translate-y-1/2 w-3.5 h-3.5 text-gray-400"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <nav class="flex items-center space-x-1">
            ...
            <select
              v-if="
                labelStore.labels.value && labelStore.labels.value.length > 0
              "
              v-model="selectedLabelFilter"
              class="text-xs px-2 py-1.5 border border-gray-200 rounded-lg bg-white/60 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 dark:bg-gray-700/60 dark:border-gray-600 dark:text-gray-200"
              :title="$t('notes.labelFilter')"
            >
              <option :value="undefined">{{ $t('notes.allLabels') }}</option>
              <option value="__none__">{{ $t('notes.noLabel') }}</option>
              <option
    ...
    <main class="main-content max-w-7xl mx-auto overflow-hidden">
      <TimelineView
        ...
        :label-filter="selectedLabelFilter"
      />
    </main>
